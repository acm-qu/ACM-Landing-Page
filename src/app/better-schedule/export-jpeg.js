...
export async function exportSheetAsJpeg({ geo, theme, accent, font }) {
  ...
  for (const b of geo.blocks) {
    ...
    x.save(); x.beginPath(); x.rect(ox + b.x, oy + b.y, b.w, b.h); x.clip();
    const lines = [];
    ...
    for (const l of lines) {
      x.font = l.f; x.fillStyle = b.ink;
      const cx = ox + b.x + b.w / 2;
      const maxW = b.w - 14 - (l.icon ? 18 : 0);
      const txt = fit(l.t, maxW);
      if (l.icon) {
        const tw = x.measureText(txt).width;
        drawIcon(l.icon, cx - (tw + 18) / 2, ty - 10, b.ink);
        x.fillText(txt, cx + 9, ty);
      } else {
        x.fillText(txt, cx, ty);
      }
      ty += l.h;
    }
    x.restore();
  ...
  for (const k of geo.breaks) {
    ...
    const txt = fit(k.label, k.w - 34);
    const cx = ox + k.x + k.w / 2 + 5, cy = oy + k.y + k.h / 2;
    x.fillText(txt, cx, cy + 3.5);
    x.save(); x.translate(cx - x.measureText(txt).width / 2 - 10, cy); x.rotate(Math.PI / 4); x.fillStyle = accent; x.fillRect(-2.5, -2.5, 5, 5); x.restore();
  }
